...
function createProjectCard(project) {
    ...
    // Restored the description directly to the card, but used the snippet trimmer to keep heights even
    info.innerHTML = `
        <h3 class="card-title" style="margin-bottom: 8px;">${escapeHtml(project.title)} ${liveIndicator}</h3>
        <p style="color: var(--muted); font-size: 0.95rem; margin-bottom: 18px; line-height: 1.5;">${escapeHtml(getSnippet(project.description, 140))}</p>
...
async function loadServerProjects() {
    try {
        // Cache buster guarantees GitHub pages pulls the newest JSON
