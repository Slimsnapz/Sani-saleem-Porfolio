...
const baseProjects = [
    ...
    { id: "base-3", title: "Executive Performance Dashboards", description: "Designed dynamic group-wide dashboards and manual trade registries for tracking active mandates, execution, and key metrics.", tags: ["Power BI", "Excel"], images: [], report: null }
];
...
function createProjectCard(project) {
    ...
    info.innerHTML = `
        <h3 class="card-title">${escapeHtml(project.title)} <span class="click-arrow">↗</span></h3>
...
function openProjectModal(project) {
    ...
    const reportBtnContainer = document.querySelector("#modalReportContainer");
...
function handlePdfDownload() {
    ...
    // Native browser download from base64 string
    const link = document.createElement("a");
    link.href = activeProjectForPdf.report.content;
    link.download = activeProjectForPdf.report.name || `${activeProjectForPdf.title.replace(/\s+/g, '_')}_Report.pdf`;
...
function renderAdminProjects() {
    ...
    allProjects.forEach(project => {
        ...
        if (project.report) slideCountText.push(`1 PDF Report`);
        let slideString = slideCountText.length ? slideCountText.join(" + ") : "No files attached";
...
function resetForm() {
    ...
    document.querySelector("#reportHelpText").textContent = "Upload a native PDF report. Visitors can download this directly. Keep file sizes reasonable.";
...
function initAdmin() {
    ...
    projectForm.addEventListener("submit", async (event) => {
        ...
        // Save the PDF exactly as uploaded (as a base64 string)
        ...
        try {
            ...
            if (response.ok) {
                loadServerProjects();
                resetForm();
                document.querySelector("#projectMessage").style.color = "var(--accent)";
                document.querySelector("#projectMessage").textContent = "Project successfully saved!";
                setTimeout(() => document.querySelector("#projectMessage").textContent = "", 3000);
            }
        } catch {
            document.querySelector("#projectMessage").style.color = "#c65042";
            document.querySelector("#projectMessage").textContent = "Error saving to server.";
        }
    });
